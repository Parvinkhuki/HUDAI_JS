...
function isValidPhotoName(photoName, imageExtensions){
    if(!Array.isArray(imageExtensions)){
        return "can u please give us a ARRAY"}
        if(typeof photoName !="string"){
            return "give a string otherwise i will never give you proper ans"
        }
        
        for(i of imageExtensions){
            if(photoName.toLowerCase().endsWith(i.toLowerCase)){
                return true;
            };
...
let filename = "rira.jpg"
let imageExt=[".jpg", ".jpeg", ".png", ".gif" ,".ico", ".JPEG"]
console.log(isValidPhotoName(filename, imageExt))